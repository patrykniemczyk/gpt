...
def _merge_pair_in_sequences(
    token_sequences: List[List[int]], pair: Tuple[int, int], new_token_id: int
) -> List[List[int]]:
...
class BPETokenizer:
    ...
    def __init__(
        self, vocab_size: int = 2048, special_tokens: Optional[Dict[str, str]] = None
    ) -> None:
        if vocab_size <= 0:
    ...
    def train(self, texts: List[str], min_frequency: int = 2) -> None:
        ...
        while current_vocab_size < 256 + self.max_merges:
            ...
            if frequency < min_frequency:
                logger.info(
                    f"Stopping training: max frequency {frequency} < min_frequency {min_frequency}"
                )
                break
            ...
            if current_vocab_size % 100 == 0:
                logger.debug(
                    f"Vocabulary size: {current_vocab_size}, "
    ...
    def encode(self, text: str, add_special_tokens: bool = False) -> List[int]:
        ...
        while len(tokens) > 1 and iteration_count < max_iterations:
            ...
            tokens = _merge_pair_in_sequences(
    ...
    def save(self, path: Union[str, Path]) -> None:
        ...
        save_data = {
            ...
            "merges": {
                f"{pair[0]},{pair[1]}": token_id
                for pair, token_id in self.merges.items()
            },
            "vocab": {
                str(token_id): vocab_bytes.decode("utf-8", errors="replace")
    ...
    def tokenize(self, text: str) -> List[str]:
        ...
        return [
            self.decode([token_id], skip_special_tokens=False) for token_id in token_ids
        ]
