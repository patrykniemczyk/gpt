...
class Trainer:
    ...
    def __init__(
        ...
        self.scaler = (
            torch.cuda.amp.GradScaler() if config.training.mixed_precision else None
        )
    ...
    def train(
        ...
        try:
            for epoch in range(self.current_epoch, self.config.training.epochs):
                ...
                if (
                    eval_metrics
                    and eval_metrics.get("eval_loss", float("inf"))
                    < self.best_eval_loss
                ):
    ...
    def _log_epoch_results(
        ...
        if eval_metrics:
            logger.info(
                f"  Eval  - Loss: {eval_metrics.get('eval_loss', 0):.4f}, "
